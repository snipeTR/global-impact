...
| **Siyasi sermaye** | Her müdahale puan harcar. Toplum onayı yüksekse daha çabuk dolar. |
...
│       ├── panels.js       # paneller, enstrüman UI, sayfalama
│       ├── screens.js      # menü, harita pan/zoom/touch
...
```text
countries → instruments → disasters
→ state → effects → internal → diplomacy → news → ai → turn
→ charts → help → music → panels → screens → mobile → main
```

Sırayı bozma: `main` her şeye, `mobile` panellere, `ai` effects’e bağlıdır.
...
| Deploy secret’ı README’ye yazmak | Yalnız yerel `YAPILACAKLAR.md` |
